...
const PayInScreen = () => {
  ...
  useEffect(() => {
    ...
    console.log(amount, "Deep link amount");
    if (deepLinkAmount) {
      setAmount(deepLinkAmount);
    }
  }, []);
  console.log(amount, "Deep link amount after setting");
  ...
  const callPaymentGateway = async () => {
    try {
      const response = await axios.post(
        'http://192.168.113.177:5000/initiate-payment',
        {
          txnid: txnId,
          amount: amount,
      ...
      EasebuzzCheckout.open(options)
        .then(async (data) => {
          ...
          try {
            await axios.post('http://192.168.113.177:5000/api/save', {
